...
_registered_deployments() {
    # This is used for checking is _deployments.sh has been imported or not
    return 0
}
...
_download_project_files() {
    ...
    if git clone --quiet "${GIT_URL}" "${GIT_REPO_NAME}_${NOWDATESTAMP}_clone"; then # If failed to git clone
        ...
        mv -u -f "$HOME/${GIT_REPO_NAME}_${NOWDATESTAMP}_clone" "$HOME/$GIT_REPO_NAME"
...
_setup() {
    local _ask_to_edit_secret=false

    _install_linux_dep

    if [[ ! -f "$HOME/.${GIT_REPO_NAME}" ]]; then
        _ask_to_edit_secret=true
    fi

    _ensure_project_secrets_file

    # Check if the file exists
    if [[ "$(_interactive_shell)" = "1" && "$_ask_to_edit_secret" = "true" && -f "$HOME/.${GIT_REPO_NAME}" ]]; then
        # Ask the user if they want to edit the file
        echo ""
        read -p "Created secrets file, would you like to editor now? (y/n): " response
        case "$response" in
        [Yy]*)
            # Open the file using the editor, default to nano if EDITOR is not set
            ${EDITOR:-nano} "$HOME/.${GIT_REPO_NAME}"
            ;;
        *)
